...
export function createEventPages(api) {
  ...
  byId('exportDoorCheckins').addEventListener('click', () => api.exportCheckins());
  return function sync(data) {
    ...
    if (page === 'vendas-online' && !data.manager) page = 'resumo';
    if (page === 'portaria' && data.promoter) page = 'resumo';
    ...
    byId('pageOnlineSales').hidden = page !== 'vendas-online';
    ...
    document.title = `${home ? 'Meus eventos' : page === 'arquivados' ? 'Eventos arquivados' : page === 'configuracao-ingresso' ? 'Configuração do ingresso' : page === 'gerenciar-usuarios' ? 'Gerenciar usuários' : page === 'promoters' ? 'Promoters' : page === 'vendas-online' ? 'Vendas online' : links.find(l => l[0] === page)?.[2] || 'Evento'}${!globalPage ? ` — ${data.event.name}` : ''} | Le Beef`;
