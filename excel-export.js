...
(() => {
  ...
  window.exportSalesXlsx = (sales, events, eventId) => {
    ...
    const rows = selectedSales.flatMap((sale) => saleItems(sale).map((item) => { const courtesy = sale.courtesy || sale.paymentMethod === "courtesy" || /^CORTESIA:/i.test(item.ticketTypeName); const reservationDetails = sale.reservationType === "table" ? `${sale.reservationLabel || "Reserva de mesa"}${Array.isArray(sale.occupants) && sale.occupants.length ? ` | Ocupantes: ${sale.occupants.join(", ")}` : ""}` : ""; const observation = [reservationDetails, sale.notes || ""].filter(Boolean).join(" | "); return [eventName(sale.eventId), item.ticketTypeName, sale.buyerName || "", sale.buyerPhone || "", observation, item.quantity, item.subtotal, courtesy ? "CORTESIA" : sale.paid ? "PAGO" : "PENDENTE", courtesy ? "CORTESIA" : sale.paid ? paymentMethods[sale.paymentMethod] || "NÃO INFORMADA" : "", courtesy ? "" : sale.paid ? paymentDate(sale.paymentDate) : "", sale.createdByName || "VENDAS ANTERIORES", sale.checkedIn ? "SIM" : "NÃO"]; }));
