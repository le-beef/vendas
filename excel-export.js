...
(() => {
  ...
  window.exportSalesXlsx = (sales, events, eventId, mode = "unit") => {
    ...
    const url = URL.createObjectURL(zip(files)); const link = document.createElement("a"); link.href = url; link.download = `${mode === "tables" ? "reservas-mesas" : "participantes"}-${safeName}.xlsx`; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000);
  };
})();
