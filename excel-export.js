...
(() => {
  ...
  window.exportSalesXlsx = (sales, events, eventId) => {
    ...
    const header = ["EVENTO", "TIPO DE INGRESSO", "PARTICIPANTE", "TELEFONE / CONTATO", "OBSERVAÇÃO", "QTD.", "VALOR", "PAGAMENTO", "ENTRADA"];
    const rows = selectedSales.map((sale) => [eventName(sale.eventId), sale.ticketTypeName || "Ingresso padrão", sale.buyerName || "", sale.buyerPhone || "", sale.notes || "", Number(sale.quantity || 0), Number(sale.total || 0), sale.paid ? "PAGO" : "PENDENTE", sale.checkedIn ? "SIM" : "NÃO"]);
    ...
    const sheet = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><worksheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><cols><col min="1" max="1" width="25" customWidth="1"/><col min="2" max="2" width="23" customWidth="1"/><col min="3" max="3" width="28" customWidth="1"/><col min="4" max="4" width="21" customWidth="1"/><col min="5" max="5" width="34" customWidth="1"/><col min="6" max="6" width="9" customWidth="1"/><col min="7" max="7" width="16" customWidth="1"/><col min="8" max="9" width="18" customWidth="1"/></cols><sheetData>${sheetRows.join("")}</sheetData></worksheet>`;
