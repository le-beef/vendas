...
(() => {
  ...
  window.exportSalesXlsx = (sales, events, eventId) => {
    ...
    const rows = selectedSales.map((sale) => [eventName(sale.eventId), sale.ticketTypeName || "Ingresso padrão", sale.buyerName || "", sale.buyerPhone || "", sale.notes || "", Number(sale.quantity || 0), Number(sale.total || 0), sale.paid ? "PAGO" : "PENDENTE", sale.paid ? paymentMethods[sale.paymentMethod] || "NÃO INFORMADA" : "", sale.paid ? paymentDate(sale.paymentDate) : "", sale.createdByName || "VENDAS ANTERIORES", sale.checkedIn ? "SIM" : "NÃO"]);
