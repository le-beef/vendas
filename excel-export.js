...
(() => {
  ...
  window.exportSalesXlsx = (sales, events, eventId, mode = "unit") => {
    ...
    const allSelectedSales = (eventId ? sales.filter((sale) => sale.eventId === eventId) : sales).filter((sale) => sale.reservationType !== "table_block" && sale.nonRevenue !== true && sale.onlinePending !== true);
    ...
    const channelLabel = (sale) => sale.channel === "online" ? "ONLINE" : "MANUAL";
    const saleSubtotal = (sale) => Number(sale.subtotal ?? saleItems(sale).reduce((sum, item) => sum + Number(item.subtotal || 0), 0));
    const commercialColumns = (sale) => [channelLabel(sale), saleSubtotal(sale), Number(sale.buyerFeeAmount || 0), Number(sale.total ?? saleSubtotal(sale) + Number(sale.buyerFeeAmount || 0)), sale.mercadoPagoOrderId || ""];
    ...
    const header = mode === "checkins"
      ? ["EVENTO", "MODALIDADE", "ÁREA", "MESA / BISTRÔ", "PARTICIPANTE", "RESPONSÁVEL", "TELEFONE", "TIPO DE INGRESSO", "DESCONTO", "QTD.", "VALOR", "PAGAMENTO", "FORMA DE PAGAMENTO", "DATA DO PAGAMENTO", "VENDEDOR", "OBSERVAÇÃO", "ENTRADA", "DATA / HORA DO CHECK-IN", "QR CODE", "CÓDIGO", "LINK DE VALIDAÇÃO", "CANAL", "SUBTOTAL DA VENDA", "TAXA DO COMPRADOR", "TOTAL DA VENDA", "PEDIDO MERCADO PAGO"]
      : mode === "tables"
      ? ["EVENTO", "ÁREA", "MESA / BISTRÔ", "RESPONSÁVEL", "TELEFONE", "OCUPANTES", "DESCONTOS DAS CADEIRAS", "QTD.", "VALOR", "PAGAMENTO", "FORMA DE PAGAMENTO", "DATA DO PAGAMENTO", "VENDEDOR", "OBSERVAÇÃO", "ENTRADAS", "CANAL", "SUBTOTAL DA VENDA", "TAXA DO COMPRADOR", "TOTAL DA VENDA", "PEDIDO MERCADO PAGO"]
      : ["EVENTO", "TIPO DE INGRESSO", "PARTICIPANTE", "TELEFONE / CONTATO", "OBSERVAÇÃO", "QTD.", "VALOR", "PAGAMENTO", "FORMA DE PAGAMENTO", "DATA DO PAGAMENTO", "VENDEDOR", "ENTRADA", "CANAL", "SUBTOTAL DA VENDA", "TAXA DO COMPRADOR", "TOTAL DA VENDA", "PEDIDO MERCADO PAGO"];
    const rows = mode === "checkins"
      ? selectedSales.flatMap(checkinRows)
      : mode === "tables"
      ? selectedSales.map((sale) => [eventName(sale.eventId), (sale.mapArea || sale.reservationArea) === "mezanino" ? "MEZANINO" : "SALÃO", sale.reservationLabel || "RESERVA", sale.buyerName || "", sale.buyerPhone || "", reservationOccupants(sale).join(", "), reservationDiscounts(sale), Number(sale.quantity || 0), Number(sale.total || 0), sale.paid ? "PAGO" : "PENDENTE", sale.paid ? paymentMethods[sale.paymentMethod] || "NÃO INFORMADA" : "", sale.paid ? paymentDate(sale.paymentDate) : "", sale.createdByName || "VENDAS ANTERIORES", sale.notes || "", reservationCheckins(sale), ...commercialColumns(sale)])
      : selectedSales.flatMap((sale) => saleItems(sale).map((item) => { const courtesy = sale.courtesy || sale.paymentMethod === "courtesy" || /^CORTESIA:/i.test(item.ticketTypeName); return [eventName(sale.eventId), item.ticketTypeName, sale.buyerName || "", sale.buyerPhone || "", sale.notes || "", item.quantity, item.subtotal, courtesy ? "CORTESIA" : sale.paid ? "PAGO" : "PENDENTE", courtesy ? "CORTESIA" : sale.paid ? paymentMethods[sale.paymentMethod] || "NÃO INFORMADA" : "", courtesy ? "" : sale.paid ? paymentDate(sale.paymentDate) : "", sale.createdByName || "VENDAS ANTERIORES", sale.checkedIn ? "SIM" : "NÃO", ...commercialColumns(sale)]; }));
    const sheetRows = [`<row r="1">${header.map((cell, i) => textCell(`${column(i)}1`, cell, 1)).join("")}</row>`];
    rows.forEach((row, index) => { const r = index + 2, style = index % 2 ? 2 : 0, moneyStyle = index % 2 ? 4 : 3, quantityIndex = mode === "checkins" ? 9 : mode === "tables" ? 7 : 5, moneyIndices = mode === "checkins" ? [10,22,23,24] : mode === "tables" ? [8,16,17,18] : [6,13,14,15]; sheetRows.push(`<row r="${r}">${row.map((cell, i) => i === quantityIndex ? numberCell(`${column(i)}${r}`, cell, style) : moneyIndices.includes(i) ? numberCell(`${column(i)}${r}`, cell, moneyStyle) : textCell(`${column(i)}${r}`, cell, style)).join("")}</row>`); });
    const widths = mode === "checkins" ? [25,16,14,19,28,28,20,34,18,8,15,14,22,18,25,34,12,24,13,14,58,12,18,18,18,28] : mode === "tables" ? [25,24,28,28,21,40,34,9,16,15,23,21,25,34,24,12,18,18,18,28] : [25,49,28,21,34,9,16,15,23,21,25,14,12,18,18,18,28];
