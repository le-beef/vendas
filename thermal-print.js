...
function ticketMarkup({ event, sale, ticket, qrCode, design, index, count, showCutGuide }) {
  ...
  const generationOrigin = `<p class="generation-origin">Gerado por: ${html(ticket.generatedByName || "Usuário não identificado")} | ${html(ticket.generatedAtText || "Data e hora não registradas")}</p>`;
  const cutGuide = showCutGuide ? `<div class="cut-guide" aria-label="Linha de corte"><div class="cut-guide-spacing" aria-hidden="true"></div><div class="cut-guide-marker"><svg class="cut-guide-icon" viewBox="0 0 36 24" aria-hidden="true" focusable="false"><circle cx="7" cy="6.5" r="4.2"/><circle cx="7" cy="17.5" r="4.2"/><path d="M10.6 8.6 31.5 1.8M10.6 15.4l20.9 6.8M10.6 8.6 19 12l-8.4 3.4"/></svg></div></div>` : "";
...
export function buildThermalPrintHtml({ event, sale, tickets, qrCodes, paperWidth, paperHeight, ticketDesign = {} }) {
  ...
  const cutSpacing = showCutGuide ? 10 : 0;
