...
function render() {
  ...
  $("salesList").innerHTML = visibleSales.length ? visibleSales.slice(0, 7).map((sale) => `<tr class="sales-row" data-sale-row="${sale.id}"><td><strong>${escapeHtml(sale.buyerName)}</strong><small>${sale.quantity} ingresso${sale.quantity > 1 ? "s" : ""}</small><span class="participant-mobile-actions">${participantContactHtml(sale, selectedEvent?.name)}<span class="mobile-checkin-control">${checkinControl(sale)}</span></span></td><td>${escapeHtml(sale.ticketTypeName || "Ingresso padrão")}</td><td class="sale-observation">${escapeHtml(sale.notes || "Sem observação")}</td><td class="financial-column mobile-value-payment">${money.format(sale.total)}<span class="mobile-payment-control"><small>Pagamento</small>${paymentControl(sale)}</span></td><td class="financial-column mobile-original-payment">${paymentControl(sale)}</td><td class="mobile-original-entry">${checkinControl(sale)}</td><td>${actionControl(sale)}</td></tr>`).join("") : `<tr><td colspan="7" class="empty">${participantSearchQuery || activeFilterCount ? "Nenhum participante encontrado com esses filtros." : "Nenhuma venda neste evento."}</td></tr>`;
