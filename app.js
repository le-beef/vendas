...
async function saveEvent(data, id = "") {
  ...
  if (isDemo) { if (id) state.events = state.events.map((item) => item.id === id ? { ...item, ...eventData } : item); else state.events.push({ id: crypto.randomUUID(), ...eventData, createdAt: Date.now() }); persistDemo(); render(); }
  else if (id) await update(ref(db, `events/${id}`), eventData); else await set(push(ref(db, "events")), { ...eventData, createdAt: Date.now() });
...
document.querySelectorAll("[data-open]").forEach((button) => button.addEventListener("click", () => { if (button.dataset.open === "eventModal") return openNewEvent(); if (button.dataset.open === "saleModal") return openNewSale(); $(button.dataset.open).showModal(); }));
...
document.addEventListener("click", (event) => { const removeTicket = event.target.closest("[data-remove-ticket]"); if (removeTicket) { if (document.querySelectorAll(".ticket-type-row").length === 1) return toast("O evento precisa de pelo menos um tipo de ingresso."); removeTicket.closest(".ticket-type-row").remove(); return; } const newSaleButton = event.target.closest("[data-new-sale]"); if (newSaleButton) { openNewSale(newSaleButton.dataset.newSale); return; } const exportEventButton = event.target.closest("[data-export-event]"); if (exportEventButton) { window.exportSalesXlsx(state.sales, state.events, exportEventButton.dataset.exportEvent); return; } const deleteEventButton = event.target.closest("[data-delete-event]"); if (deleteEventButton) { deleteEvent(deleteEventButton.dataset.deleteEvent); return; } const deleteSaleButton = event.target.closest("[data-delete-sale]"); if (deleteSaleButton) { deleteSale(deleteSaleButton.dataset.deleteSale); return; } const checkin = event.target.closest("[data-checkin]"); if (checkin) { toggleCheckin(checkin.dataset.checkin); return; } const paid = event.target.closest("[data-paid]"); if (paid) { togglePayment(paid.dataset.paid); return; } const saleRow = event.target.closest("[data-sale-row]"); if (saleRow) { openEditSale(saleRow.dataset.saleRow); return; } const card = event.target.closest("[data-event]"); if (card) openEditEvent(card.dataset.event); });
